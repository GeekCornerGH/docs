...
```
Zodra de verandering is aangebracht, voer u `npm i` uit in een Terminal (Terminal --> Nieuwe Terminal)

:::note
Elke keer dat u het `package.json` bestand aanpast, moet u het `npm i` commando opnieuw uitvoeren.
:::
...
{label: "Linux", value:"linux"}
  ...
  <TabItem value="windows"><code>%APPDATA%\Mijn Launcher\</code></TabItem>
  <TabItem value="macos"><code>~/Library/Application Support/Mijn Launcher/</code></TabItem>
