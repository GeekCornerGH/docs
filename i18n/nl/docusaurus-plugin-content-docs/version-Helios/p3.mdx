---
title: Deel 3 - Het beginscherm
---

## Introduction
In dit deel zullen we het beginscherm en de links naar sociale netwerken veranderen.

## Video
Deze tutorial is gebaseerd op de onderstaande video (**in het Frans**)
<iframe width="713" height="412" src="https://www.youtube-nocookie.com/embed/HsUwhKvBjpc" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

## Vertaal de homepage
Ga naar `app/landing.ejs`. Vertaal naar behoefte. Hier is mijn vertaling (**naar het Nederlands**)
```diff showLineNumbers title="app/landing.ejs" {6,7,15,16,18,19,29,30,103-106,110,111,120,121,144,145,153,154,156,157,166,167,199,200,221,222,225-228,231,232}
...
+                <div id="updateAvailableTooltip">Er is een update beschikbaar</div>
            </div>
        </div>
        <div id="content">
        </div>
...
+                        <button id="avatarOverlay">Bewerk</button>
                    </div>
                </div>
                <div id="mediaContent">
...
+                                <div id="settingsTooltip">Instellingen</div>
                            </button>
                        </div>
                    </div>
...
+                                <div id="mojangStatusNETitle">Non&nbsp;Essentieel</div>
                                <div class="mojangStatusNEBar"></div>
                            </div>
...
+                        &#10;<span id="newsButtonText">NIEUWS</span>
                    </button>
                </div>
            </div>
        </div>
...
+                    <button id="launch_button">SPELEN</button>
                    <div class="bot_divider"></div>
-                    <button id="server_selection_button" class="bot_label">&#8226; No Server Selected</button>
+                    <button id="server_selection_button" class="bot_label">&#8226; Geen server geselecteerd</button>
                </div>
...
+                        <span id="launch_details_text" class="bot_label">Wacht even..</span>
                    </div>
                </div>
            </div>
        </div>
...
+                <span id="nELoadSpan" class="newsErrorContent">Op zoek naar nieuws...</span>
            </div>
            <div id="newsErrorFailed" style="display: none;">
-                <span id="nEFailedSpan" class="newsErrorContent">Failed to Load News</span>
+                <span id="nEFailedSpan" class="newsErrorContent">Kan het nieuws niet laden</span>
-                <button id="newsErrorRetry">Try Again</button>
+                <button id="newsErrorRetry">Probeer het nog eens.</button>
            </div>
            <div id="newsErrorNone" style="display: none;">
-                <span id="nENoneSpan" class="newsErrorContent">No News</span>
+                <span id="nENoneSpan" class="newsErrorContent">Geen nieuws beschikbaar</span>
            </div>
        </div>
    </div>
...
## Sociale netwerken
In de vorige stap hadden we een div-element met de identifier `mediaContent`. Hierin worden de sociale netwerken geplaatst.
Als ik bijvoorbeeld YouTube wil veranderen en de link van mijn kanaal wil plaatsen, zijn hier mijn wijzigingen:

```diff title="app/landing.ejs" {2-3}
<div class="mediaContainer">
-    <a href="#" class="mediaURL" id="youtubeURL" disabled>
+    <a href="https://youtube.com/GeekCornerYT" class="mediaURL" id="youtubeURL">
        <svg id="youtubeSVG" class="mediaSVG" viewBox="35.34 34.3575 70.68 68.71500">
            <g>
                <path d="M84.8,69.52,65.88,79.76V59.27Zm23.65.59c0-5.14-.79-17.63-3.94-20.57S99,45.86,73.37,45.86s-28,.73-31.14,3.68S38.29,65,38.29,70.11s.79,17.63,3.94,20.57,5.52,3.68,31.14,3.68,28-.74,31.14-3.68,3.94-15.42,3.94-20.57"/>
            </g>
...
```
Ik heb eigenlijk de `#` vervangen door de url van mijn YouTube-kanaal, en gewoon de `disabled` verwijderd om de knop klikbaar te maken.
Om YouTube te verwijderen, is dit wat ik in mijn voorbeeld zou hebben gedaan.
```diff title="app/landing.ejs"
-<div class="mediaContainer">
-    <a href="#" class="mediaURL" id="youtubeURL" disabled>
-        <svg id="youtubeSVG" class="mediaSVG" viewBox="35.34 34.3575 70.68 68.71500">
-            <g>
...
## Mijn skin is zonder shirt!!!
Geen paniek! Breng onder app/assets/js/scripts/landing.js de volgende wijziging aan (zou op de nieuwste versies standaard opgenomen moeten zijn):
```diff title="app/assets/js/scripts/landing.js"
...
## Mojang status (ervaren ontwikkelaars)

U moet een functie maken op basis van [deze regels](https://github.com/dscalzi/helios-core/blob/master/lib/mojang/rest/MojangRestAPI.ts#L84-L152), die de eigenschap `naam` vertaalt.

## Verander server status
Nog steeds in `app/assets/js/scripts/landing.js`, wijzig deze keer de volgende regels zoals nodig:
```diff title="app/assets/js/scripts/landing.js"
...
npm start
```
Als de homepage vertaald is, is het perfect!

## Stuur wijzigingen naar GitHub
Ga terug naar GitHub Desktop, en vul het formulier linksonder in, en geef aan welke veranderingen je gemaakt hebt (of niet, maar daar zijn deze velden in principe voor), en druk dan op `Commit to master`. Druk dan op `Push origin` bovenaan.