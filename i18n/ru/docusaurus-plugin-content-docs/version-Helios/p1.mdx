...
```
После внесения изменений запустите `npm i` в терминале (Терминал --> Новый терминал)

:::note
Каждый раз, когда вы изменяете файл `package.json`, вам придется снова выполнять команду `npm i`.
:::
