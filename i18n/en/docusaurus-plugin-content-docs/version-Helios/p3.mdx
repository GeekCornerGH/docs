...
## Video
This tutorial is based on the video below
...
Go to `app/landing.ejs`. Translate according to your needs. Here is my translation (**to French**)
...
## Social networks
In the previous step, we had a div element with the identifier `mediaContent`. This is where the social networks are placed.
...
```
I actually replaced the `#` with the url of my YouTube channel, and simply removed the `disabled` to make the button clickable.
...
You will need to create a function based on [these lines](https://github.com/dscalzi/helios-core/blob/master/lib/mojang/rest/MojangRestAPI.ts#L84-L152), which translates the `name` property.

## Change server status
Still in `app/assets/js/scripts/landing.js`, this time modify the following lines as needed:
```diff title="app/assets/js/scripts/landing.js"
