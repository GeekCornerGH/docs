---
title: Partie 3 - L'écran d'accueil
---

## Introduction
Dans cette partie, nous allons changer l'écran d'accueil et les liens des réseaux sociaux.
...
Rendez-vous sous `app/landing.ejs`. Traduisez selon vos besoins. Voici pour ma part ma traduction
...
## Les réseaux sociaux
Lors de l'étape précédente, nous avions un élement div comportant l'identifiant `mediaContent`. C'est là que sont disposés les réseaux sociaux.
Par exemple, si je veux changer YouTube et mettre le lien de ma chaîne, voici mes modifications:
...
```
J'ai en fait remplacé le `#` par l'url de ma chaîne YouTube, et ai simplement supprimé le `disabled` afin de rendre le bouton cliquable.
Pour supprimer YouTube, voici ce qu'il aurait fallu faire dans mon exemple.
...
## Mon skin est torse nu!!!
Pas de panique! Sous `app/assets/js/scripts/landing.js`, faites la modification suivante (devrait être incluse de base sur les dernières versions): 
...
## Les status Mojang (développeurs expérimentés)

Il vous faudra créer une fonction basée sur [ces lignes](https://github.com/dscalzi/helios-core/blob/master/lib/mojang/rest/MojangRestAPI.ts#L84-L152), qui traduit la propriété `name`.

## Changer le status du serveur
Toujours dans `app/assets/js/scripts/landing.js`, modifiez cette fois-ci les lignes suivantes selon vos besoins:
```diff title="app/assets/js/scripts/landing.js"
...
npm start
```
Si la page d'accueil est traduite, c'est parfait!
